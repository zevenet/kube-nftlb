...
GOOS=linux go build -o ./docker/client/app ./cmd/app
...
docker build -t client docker/client --build-arg KEY=$key

# STEP 3:
#   The daemon container will be created using its Dockerfile.
#   It will be made for Docker, not for Minikube (this will come later).
docker build -t daemon docker/daemon --build-arg KEY=$key
